...
class World():
    ...
    def __init__(self, numberOfWumpus, numberOfGold, numberOfPits, map_size):

        # Import boundaries of the world. because we index from 0,
        # these are one less than the number of rows and columns.
        self.maxX = map_size - 1
        self.maxY = map_size - 1

        # Wumpus
        #Generates random positions for the Wumpus
        self.wLoc = []
        for i in range(numberOfWumpus):
            #self.wLoc.append(utils.pickRandomPose(self.maxX, self.maxY))
            pose_w = utils.Pose()
            pose_w.x = 8
            pose_w.y = 8
            self.wLoc.append(pose_w)

        # Link
        #Generates random starting position for Link
    ...
    #What the heck are these three functions
    ...
    def isEnded(self):
        dead = False
        won = False
        # Has Link met the Wumpus? -iterates through every single wumpus then check if they're at the same location
    ...
    def updateLink(self, direction):
        ...
        #so even if link picks North for example, there is still a chance, Link might move to the E or W!
    ...
    #In other words use the stochasticity of the action probabilities
    ...
    # Is the given location windy? Basically if its next to the pit
    def isWindy(self, location):
     ...
     # Does the given location glitter? If its next to the glitter
